...
class SafUtilPlugin :
    ...
    private fun onActivityResult(
        ...
        try {
            ...
            } else if (requestCode == requestCodeOpenFiles || requestCode == requestCodePickMedia) {
                // Handle the result of file/media picker.
                if (resultCode == Activity.RESULT_OK && data != null) {
                    val allowMultiple =
                        if (requestCode == requestCodePickMedia) {
                            (pendingArguments as? PendingMediaArguments)?.multiple ?: false
                        } else {
                            true
                        }
                    val uris: List<Uri> =
